...
class XPS:
    ...
    def GroupMoveAbsolute(self, socketId, GroupName, TargetPosition):
        strTargetPositions = ','.join([str(t) for t in TargetPosition])
        command = f"GroupMoveAbsolute({GroupName},{strTargetPositions})"
        return self.Send(socketId, command)
    ...
    def GroupPositionCurrentGet(self, socketId, GroupName, nbElement):
        axes_out = ",".join(['double *' for i in range(nbElement)])
        command = f"GroupPositionCurrentGet({GroupName},{axes_out})"
        error, returnedString = self.Send(socketId, command)
        if (error != 0):
            return [error, returnedString]

        return [error, *returnedString.split(",")]
    ...
    def PositionerDACOffsetDualSet(self, socketId, PositionerName, PrimaryDACOffset1, PrimaryDACOffset2,
                                   SecondaryDACOffset1, SecondaryDACOffset2):
    ...
    def HexapodCoordinatesGet(self, socketId, GroupName, CoordinateSystemIn, CoordinateSystemOut,
                              Xin, Yin, Zin, Uin, Vin, Win):
        axes_in = ",".join([str(val) for val in [Xin, Yin, Zin, Uin, Vin, Win]])
        axes_out = ",".join(['double *' for i in range(6)])
        command = f"HexapodCoordinatesGet({GroupName},{CoordinateSystemIn},{CoordinateSystemOut},{axes_in},{axes_out})"
        error, returnedString = self.Send(socketId, command)
        if (error != 0):
            return [error, returnedString]
    ...
    def HexapodCoordinateSystemGet(self, socketId, GroupName, CoordinateSystem):
        axes_out = ",".join(['double *' for i in range(6)])
        command = f"HexapodCoordinateSystemGet({GroupName},{CoordinateSystem},{axes_out})"
        error, returnedString = self.Send(socketId, command)
        if (error != 0):
            return [error, returnedString]
    ...
    def HexapodMoveAbsolute(self, socketId, GroupName, X, Y, Z, U, V, W):
        move = ",".join([str(val) for val in [X, Y, Z, U, V, W]])
        command = f"HexapodMoveAbsolute({GroupName},Work,{move})"
        error, returnedString = self.Send(socketId, command)
        if (error != 0):
            return [error, returnedString]
    ...
    def HexapodMoveIncremental(self, socketId, GroupName, CoordinateSystem, X, Y, Z, U, V, W):
        move = ",".join([str(val) for val in [X, Y, Z, U, V, W]])
        command = f"HexapodMoveIncremental({GroupName},{CoordinateSystem},{move})"
        error, returnedString = self.Send(socketId, command)
        if (error != 0):
            return [error, returnedString]
    ...
    def HexapodMoveIncrementalControl(self, socketId, GroupName, CoordinateSystem, HexapodTrajectoryType, dX, dY, dZ):
        delta_move = ",".join([str(val) for val in [dX, dY, dZ]])
        command = f"HexapodMoveIncrementalControl({GroupName},{CoordinateSystem},{HexapodTrajectoryType},{delta_move})"
    ...
    def HexapodMoveIncrementalControlWithTargetVelocity(self, socketId, GroupName, CoordinateSystem,
        ...
        delta_move = ",".join([str(val) for val in [dX, dY, dZ]])
        func = 'HexapodMoveIncrementalControlWithTargetVelocity'
        command = f"{func}({GroupName},{CoordinateSystem},{HexapodTrajectoryType},{delta_move})"
    ...
    def HexapodMoveIncrementalControlLimitGet(self, socketId, GroupName, CoordinateSystem,
                                              HexapodTrajectoryType, dX, dY, dZ, Velocity):
        delta_move = ",".join([str(val) for val in [dX, dY, dZ]])
        func = 'HexapodMoveIncrementalControlLimitGet'
        command = f"{func}({GroupName},{CoordinateSystem},{HexapodTrajectoryType},{delta_move})"
    ...
    def HexapodMoveIncrementalControlPulseAndGatheringSet(self, socketId, GroupName, Divisor):
        func = 'HexapodMoveIncrementalControlPulseAndGatheringSet'
        command = f"{func}({GroupName},{Divisor})"
    ...
    def HexapodSGammaParametersDistanceGet(self, socketId, FullPositionerName, Velocity, Displacement, Acceleration,
                                           MinimumJerkTime, MaximumJerkTime):

        func = 'HexapodSGammaParametersDistanceGet'
        command = f"{func}({FullPositionerName},{Velocity},{Displacement}," \
                  f"{Acceleration},{MinimumJerkTime},{MaximumJerkTime})"
